...
it('sem divergência retorna vazio', () => {
  const lock = {
    kizunaCore: { version: '0.5.0', sha: 'abc' },
...
it('bump de versão aparece no banner', () => {
  const lock = {
    kizunaCore: { version: '0.4.0', sha: 'abc' },
  ...
  const out = renderDivergence({
    ...
    liveSha: 'def',
    liveTemplateVersion: '0.5.0',
    pluginDeltas: [{ name: 'messaging', from: 4, to: 6 }],
    logText: '- feat(chat): rotas',
  });
