...
it('funde shell de plugin habilitado', () => {
  ...
  writeFileSync(join(core, 'plugins', 'agenda', 'shell', 'manifest.json'), JSON.stringify({
    owner: 'agenda',
...
it('lança DuplicateOwnerError em path com dois donos', () => {
  ...
  writeFileSync(join(core, 'plugins', 'x', 'shell', 'manifest.json'), JSON.stringify({
    owner: 'x', files: { 'app/proxy.ts': 'managed' },
  }));
