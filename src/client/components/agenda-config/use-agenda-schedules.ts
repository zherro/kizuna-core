...
export function useAgendaSchedules() {
  ...
  const [schedules, setSchedules] = useState<ScheduleWithHours[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const schedList =
        (await readJson<ListResponse<ScheduleRecord & { id: string }>>(schedRes))?.items ?? [];
      const hoursList = (await readJson<ListResponse<ScheduleHourRecord>>(hoursRes))?.items ?? [];

      const bySchedule = new Map<string, ScheduleHourRecord[]>();
      ...
      setSchedules(
        schedList.map((s) => ({ ...s, hours: mergeHoursByDay(bySchedule.get(s.id) ?? []) }))
      );
    } catch {
      toastError('Não foi possível carregar seus horários.');
    } finally {
  ...
  const saveSchedule = useCallback(
    async (draft: {
      ...
      try {
        ...
        const schedPayload =
          await readJson<ItemResponse<ScheduleRecord & { id: string }>>(schedRes);
        ...
        const savedHours: ScheduleHourRecord[] = [];
        for (const h of normalized) {
          ...
          const payload = await readJson<ItemResponse<ScheduleHourRecord>>(res);
  ...
  const setScheduleActive = useCallback(
    async (id: string, active: boolean): Promise<boolean> => {
      ...
      try {
        ...
        const payload = await readJson<ItemResponse<ScheduleRecord & { id: string }>>(res);
        if (!res.ok || !payload?.item) {
  ...
  const removeSchedule = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      try {
        ...
        if (!res.ok) {
          const payload = await readJson<{ message?: string }>(res);
  ...
  const duplicateSchedule = useCallback(
    async (source: ScheduleWithHours): Promise<boolean> => {
      return saveSchedule({
