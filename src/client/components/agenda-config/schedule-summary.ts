...
import { DAY_SHORT, LUNCH_DAY_OF_WEEK, WEEK_DAYS, type ScheduleHourRecord } from './types';
...
export function validateSchedule(name: string, hours: ScheduleHourRecord[]): ValidationResult {
  if (!name.trim()) return { ok: false, message: 'Dê um nome para esta disponibilidade.' };
...
function groupConsecutive(days: number[]): number[][] {
...
export function summarizeSchedule(hours: ScheduleHourRecord[]): string {
  ...
  for (const [window, days] of byWindow) {
    ...
    const label = groupConsecutive(days)
      .map((g) =>
        g.length === 1 ? DAY_SHORT[g[0]] : `${DAY_SHORT[g[0]]}–${DAY_SHORT[g[g.length - 1]]}`
      )
      .join(', ');
  ...
  const lunch = hours.find((h) => h.day_of_week === LUNCH_DAY_OF_WEEK);
  if (lunch?.active) parts.push(`almoço ${lunch.open_time}–${lunch.close_time}`);

  return parts.join(' · ');
}

/** Suggested name from the shape of the week — always editable by the user. */
export function suggestScheduleName(hours: ScheduleHourRecord[]): string {
  const active = hours.filter((h) => h.day_of_week !== LUNCH_DAY_OF_WEEK && h.active);
  if (active.length === 0) return 'Nova disponibilidade';
  ...
  const latest = Math.max(
    ...active.map((h) =>
      crossesMidnight(h.open_time, h.close_time)
  ...
  if (span >= 1439) return 'Atendimento 24h';
  if (isEveryDay) return 'Todos os dias';
  if (weekendOnly) return 'Fim de semana';

  const nightly = earliest >= 18 * 60 || latest > 24 * 60;
  if (isMonFri && nightly) return 'Atendimento noturno';
  if (isMonFri) return 'Horário comercial';
  if (nightly) return 'Atendimento noturno';
