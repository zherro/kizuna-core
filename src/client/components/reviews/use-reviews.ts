...
function pick<T>(r: RawRecord, camel: string, snake: string, fallback: T): T {
  const v = r[camel] ?? r[snake];
  return (v === undefined || v === null ? fallback : v) as T;
}

function coerceTags(raw: unknown): { slug: string; label: string }[] {
  return asArray(raw)
    .map((entry) => {
      if (!entry || typeof entry !== 'object') return null;
      const e = entry as RawRecord;
...
export function coerceReview(raw: unknown, currentUserId: string | null): ReviewView | null {
  const record = (Array.isArray(raw) ? raw[0] : raw) as RawRecord | null;
  if (!record || typeof record !== 'object') return null;

  const ownerId = String(
    record.idCustomer ?? record.id_customer ?? record.authorId ?? record.author_id ?? ''
  );
  const explicitMine = record.isMine ?? record.is_mine;
  const isMine =
    typeof explicitMine === 'boolean'
      ? explicitMine
  ...
  return {
    ...
    status: (pick(record, 'status', 'status', 'published') as ReviewStatus) ?? 'published',
    isMine,
  };
}

function coerceStats(raw: unknown): ReviewStats | null {
  const record = (Array.isArray(raw) ? raw[0] : raw) as RawRecord | null;
  if (!record || typeof record !== 'object') return null;
  const distRaw = (pick(record, 'dist', 'dist', {}) ?? {}) as Record<string, unknown>;
  const dist = {
...
function coerceTagOption(raw: unknown): ReviewTagOption | null {
  ...
  return {
    ...
    domain: (pick(r, 'domain', 'domain', null) as string | null) ?? null,
...
async function readJson(response: Response): Promise<{ ok: boolean; body: RawRecord | null }> {
  const body = (await response.json().catch(() => null)) as RawRecord | null;
  return { ok: response.ok, body };
}

function messageFrom(body: RawRecord | null, fallback: string): string {
...
export function useReviewStats(domain: string, referenceId: string) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (activeRef.current) setError('Erro de conexão ao carregar o resumo.');
    } finally {
...
export function useReviewList(
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [filter, setFilterState] = useState<ReviewListFilter>(opts.filter ?? 'all');
  const [sort, setSort] = useState<ReviewListSort>(opts.sort ?? 'recent');
  const activeRef = useRef(true);
  ...
  const load = useCallback(
    async (targetPage: number) => {
      ...
      try {
        ...
        if (!includeAllStatuses) query.set('filter.status', 'published');
        if (filter !== 'all') query.set('filter.rating', filter);
        ...
        setItems(rows);
        setPage(typeof body?.page === 'number' ? (body.page as number) : targetPage);
        const rawTotal = typeof body?.total === 'number' ? (body.total as number) : rows.length;
        setTotal(Math.max(rawTotal, rows.length));
      } catch {
        if (activeRef.current) setError('Erro de conexão ao carregar as avaliações.');
      } finally {
        if (activeRef.current) setLoading(false);
      }
    },
  ...
  const setFilter = useCallback((next: ReviewListFilter) => {
    setFilterState(next);
    setPage(1);
  }, []);
...
export function useReviewTags(domain?: string) {
  const [tags, setTags] = useState<ReviewTagOption[]>([]);
  const [loading, setLoading] = useState(true);
  const activeRef = useRef(true);
...
export function useMyReview(domain: string, referenceId: string) {
  const { user } = useAuth();
  const currentUserId = user?.user_id ?? null;
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (activeRef.current) setReview(null);
    } finally {
      if (activeRef.current) setLoading(false);
    }
...
function unwrapRow(body: RawRecord | null): unknown {
  if (!body) return null;
...
export async function submitReview(input: SubmitReviewInput): Promise<ReviewView> {
  ...
  if (input.existingReviewId) {
    ...
    const { ok, body } = await readJson(res);
    if (!ok) throw new Error(messageFrom(body, 'Não foi possível atualizar sua avaliação.'));
    const parsed = coerceReview(unwrapRow(body), null);
    if (!parsed) throw new Error('Resposta inválida ao atualizar a avaliação.');
  ...
  const { ok, body } = await readJson(res);
  if (!ok) throw new Error(messageFrom(body, 'Não foi possível enviar sua avaliação.'));
  const parsed = coerceReview(unwrapRow(body), null);
  if (!parsed) throw new Error('Resposta inválida ao criar a avaliação.');
  return { ...parsed, isMine: true };
}

export async function requestModeration(reviewId: string, reason: string): Promise<void> {
...
export async function moderateReview(args: ModerateReviewArgs): Promise<ReviewView> {
  ...
  const { ok, body } = await readJson(res);
  if (!ok) throw new Error(messageFrom(body, 'Não foi possível moderar a avaliação.'));
  const parsed = coerceReview(unwrapRow(body), null);
  if (!parsed) throw new Error('Resposta inválida ao moderar a avaliação.');
  return parsed;
}
