 ...
 */
export const resourceServices: Record<string, ResourceConfig> = {
  services: {
    schema: 'public',
    table: 'services',
    ...
    mapInput: (input) => {
      ...
      const extras =
        input.extras && typeof input.extras === 'object' ? input.extras : (input.extras ?? {});
    ...
    mapOutput: (record) => {
      const categoryGroup =
        record.category_group && typeof record.category_group === 'object'
          ? (record.category_group as { id?: unknown })
          : null;
      const category =
        record.category && typeof record.category === 'object'
          ? (record.category as { id?: unknown })
          : null;
  ...
  service_categories_sub: {
    ...
    returnRepresentation: true,
    select:
      'id,service_id,category_group_id,category_id,category_sub_id,active,created_at,updated_at',
    primaryKey: 'id',
    defaultOrder: 'created_at',
    searchableColumns: [],
  ...
  service_moderations: {
    ...
    returnRepresentation: true,
    select:
      'id,uid,service_id,priority,decision,decision_note,rejection_reason,decided_at,auto_approved,active,created_by,created_at,updated_at',
    primaryKey: 'id',
    defaultOrder: 'created_at',
