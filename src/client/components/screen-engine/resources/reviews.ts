...
/**
 * `ResourceConfig`s for the `reviews` plugin tables, exposed through a consuming app's
 * `/api/resources/[resource]` proxy. Spread into the app's `postgrestResources` registry
 * (foco-total spreads `resourceReviews` in `src/lib/server/resources/index.ts`, via
 * `resource-reviews.ts`).
 *
 * Writes to `reviews` / `review_moderation_requests` / `review_moderation_events` mostly go
 * through the plugin RPCs (`fn_review_create`, `fn_review_moderation_request`,
 * `fn_review_moderate`) registered in `postgrestRpcs`, NOT these configs — these cover the
 * read paths plus the two narrow PATCH cases (author edits rating/comment; moderator resolves a
 * request). See the plugin: `kizuna-core/plugins/reviews/0001_reviews.sql` and the design doc
 * `foco-total/docs/superpowers/specs/2026-09-06-plugin-avaliacoes-design.md`.
 *
 * PRIVACY: `reviews` `mapOutput` NEVER emits `id_customer` / `tenant_id`. `authorName` is the
 * `author_name` snapshot masked to "first name + initial". RLS (not this config) decides which
 * rows a caller sees (anon → published only; author → own; `reviews.moderate` → all).
 */
...
type TagLinkRow = { tag_slug_snapshot?: unknown; tag_label_snapshot?: unknown };

function mapTagLinks(record: RecordValue): { slug: string; label: string }[] {
  const links = record.review_tag_links;
  if (!Array.isArray(links)) return [];
  return links.map((l: TagLinkRow) => ({
    slug: String(l?.tag_slug_snapshot ?? ''),
    label: String(l?.tag_label_snapshot ?? ''),
  }));
}

const REVIEWS_RESOURCE: ResourceConfig = {
  schema: 'public',
  table: 'reviews',
  listRequiresAuth: false, // published reviews are public; RLS filters the rest
  returnRepresentation: true,
  select:
    'id,uid,domain,reference_id,rating,comment,author_name,status,active,created_at,updated_at,review_tag_links(tag_slug_snapshot,tag_label_snapshot)',
  primaryKey: 'id',
...
const REVIEW_TAGS_RESOURCE: ResourceConfig = {
  schema: 'public',
  table: 'review_tags',
  ...
  mapInput: (input) => {
    ...
    if (input.slug !== undefined || input.label !== undefined) {
      ...
      if (slugSource) out.slug = makeSlug(slugSource);
    }
    ...
    if (input.selectable !== undefined) out.selectable = parseActive(input.selectable);
...
const REVIEW_MODERATION_REQUESTS_RESOURCE: ResourceConfig = {
  schema: 'public',
  table: 'review_moderation_requests',
  listRequiresAuth: true,
  returnRepresentation: true,
  select:
    'id,uid,review_id,tenant_id,requester_user_id,reason,status,moderator_id,moderator_comment,created_at,updated_at,resolved_at',
  primaryKey: 'id',
  defaultOrder: 'created_at',
...
const REVIEW_MODERATION_EVENTS_RESOURCE: ResourceConfig = {
  schema: 'public',
...
const REVIEW_STATS_RESOURCE: ResourceConfig = {
  schema: 'public',
  table: 'review_stats',
  ...
  mapOutput: (record) => {
    const dist =
      record.dist && typeof record.dist === 'object' && !Array.isArray(record.dist)
...
export const resourceReviews: Record<string, ResourceConfig> = {
