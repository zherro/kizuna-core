...
export interface AiSkill<I = unknown, L = unknown, O = unknown> {
  ...
  buildPrompt(input: I, loaded: L): { systemPrompt: string; contents: unknown };
  schema: unknown;
...
export function registerSkill(skill: AiSkill): void {
  registry.set(skill.key, skill);
}

export function getSkill(key: string): AiSkill | undefined {
  return registry.get(key);
}

/** Contextos distintos das skills registradas — usado pela tela de configuração. */
export function listSkillContexts(): string[] {
