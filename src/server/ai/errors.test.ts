...
describe('classifyAiError', () => {
  ...
  it('modelo removido → blocked', () =>
    expect(classifyAiError('model no longer available')).toBe('blocked'));
});
describe('isRecoverableAiError', () => {
  it('503 é recuperável', () =>
    expect(isRecoverableAiError(new Error('503 overloaded'))).toBe(true));
});
