...
import { postgrestResources, postgrestRpcs } from '@/lib/server/resources';
...
function parsePgError(payload) {
    ...
    const message = code === '23505'
        ...
        : (json?.message ?? 'Erro ao processar requisicao no PostgREST.');
...
function getSchemaHeaders(config, method) {
    const headers = {};
    if (!config.schema)
        return headers;
...
async function ensureAuthenticated() {
    ...
    if (!session) {
        return {
            error: NextResponse.json({ message: 'Sessao expirada. Faca login novamente.' }, { status: 401 }),
            authHeader: null,
        };
    }
    ...
    if (!authHeader) {
        return {
            error: NextResponse.json({ message: 'Nao foi possivel montar o token de autenticacao.' }, { status: 401 }),
            authHeader: null,
        };
    }
...
export async function listResource(resource, request) {
    ...
    const pageSize = Math.min(parsePositiveInt(searchParams.get('pageSize'), 20), config.maxPageSize ?? 100);
    const offset = (page - 1) * pageSize;
    const search = (searchParams.get('search') ?? '').trim();
    const orderBy = (searchParams.get('orderBy') ?? config?.defaultOrder ?? '').trim();
    const orderDirection = (searchParams.get('orderDirection') ?? 'asc').toLowerCase() === 'desc' ? 'desc' : 'asc';
    ...
    if (search && config.searchableColumns.length > 0) {
        ...
        const clauses = config.searchableColumns.map((column) => `${column}.ilike.*${normalizedSearch}*`);
    ...
    for (const [key, value] of searchParams.entries()) {
        if (key.startsWith('filter_cs.')) {
            ...
            if (field && filterValue)
                query.set(field, `cs.${filterValue}`);
            continue;
        }
    ...
    if (!response.ok) {
        ...
        return NextResponse.json({ message: err.message, details: err.details }, { status: response.status });
    }
...
export async function createResource(resource, request) {
    const authState = await ensureAuthenticated();
    if (authState.error)
        return authState.error;
    const config = getResourceConfig(resource);
    ...
    if (config.requiredFields?.length) {
        for (const field of config.requiredFields) {
            ...
            if (!value) {
                return NextResponse.json({ message: `Informe o campo obrigatorio: ${field}.` }, { status: 400 });
            }
        }
    }
    const response = await pgrstTable(`/${config.table}?select=${encodeURIComponent(config.select)}`, {
        method: 'POST',
        headers: {
    ...
    return NextResponse.json({ message: 'Registro criado com sucesso.', item: created }, { status: 201 });
}
...
export async function updateResource(resource, id, request, options = {
    skipMapInput: false,
}) {
    const authState = await ensureAuthenticated();
    if (authState.error)
...
export async function deleteResource(resource, id) {
    ...
    const response = await pgrstTable(`/${config.table}?${query.toString()}`, config.softDeleteField
        ? {
            method: 'PATCH',
            headers: {
                Prefer: 'return=representation',
 ...
 */
export async function serverFetchResource(resource, filters = {}, options = {}) {
    const config = getResourceConfig(resource);
    if (!config)
    ...
    return mapRows(config, payload);
}
export function isRpcResource(resource) {
    return Boolean(postgrestRpcs[resource]);
}
export async function executeRpcResource(resource, request) {
...
//# sourceMappingURL=postgrest-crud.js.map