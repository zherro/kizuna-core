...
function makeStoragePath(purpose, originalName) {
    const now = new Date();
...
function parsePgErrorMessage(payload, fallback) {
    const pg = payload ?? null;
...
async function listFilesPostgres(args) {
    const query = new URLSearchParams({
        select: 'id,uid,original_name,storage_path,public_url,mime_type,size_bytes,width,height,purpose,active,created_at,updated_at',
    ...
    const response = await pgrstTable(`/files?${query.toString()}`, {
        method: 'GET',
        headers: {
...
async function uploadSingleFilePostgres(args) {
    ...
    const response = await pgrstTable('/files?select=id,uid,original_name,storage_path,public_url,mime_type,size_bytes,width,height,purpose,active,created_at,updated_at', {
        method: 'POST',
        headers: {
            'Accept-Profile': 'public',
...
async function uploadFilesPostgres(args) {
    const uploaded = [];
    const errors = [];
...
async function deleteFilePostgres(args) {
    const cleanId = String(args.id ?? '').trim();
    if (!UUID_RE.test(cleanId)) {
        return false;
    }
    const response = await pgrstTable(`/files?id=eq.${cleanId}&select=id`, {
        method: 'PATCH',
        headers: {
            'Accept-Profile': 'public',
            'Content-Profile': 'public',
...
async function getFileContentPostgres(args) {
    ...
    const payload = (await response.json().catch(() => null));
    if (!response.ok) {
        throw new Error(parsePgErrorMessage(payload, 'Nao foi possivel carregar o conteudo do arquivo.'));
    }
    const rows = (Array.isArray(payload) ? payload : []);
    const found = rows[0];
    if (!found)
