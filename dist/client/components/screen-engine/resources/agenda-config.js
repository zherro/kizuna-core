...
// ---------------------------------------------------------------------------------------------
export const AGENDA_SCHEDULE_RESOURCE = {
    schema: 'public',
    ...
    mapInput: (input) => {
        ...
        if (input.active !== undefined)
            payload.active = toBool(input.active, true);
        return payload;
    },
    mapOutput: (record) => ({
        id: record.id,
...
// ---------------------------------------------------------------------------------------------
export const AGENDA_SCHEDULE_HOURS_RESOURCE = {
    schema: 'public',
    ...
    mapInput: (input) => {
        ...
        if (input.day_of_week !== undefined)
            payload.day_of_week = toInt(input.day_of_week, 0);
        if (input.open_time !== undefined)
            payload.open_time = toHm(input.open_time, '08:00');
        if (input.close_time !== undefined)
            payload.close_time = toHm(input.close_time, '18:00');
        if (input.active !== undefined)
            payload.active = toBool(input.active, true);
        return payload;
    },
...
// ---------------------------------------------------------------------------------------------
export const AGENDA_BOOKING_PREFERENCES_RESOURCE = {
    schema: 'public',
    table: 'agenda_booking_preferences',
    returnRepresentation: true,
    select: 'id,booking_window_days,client_picks_schedule,min_advance_hours,buffer_minutes,service_radius_km,auto_accept_trusted,active,tenant_id,created_by,created_at,updated_at',
    primaryKey: 'id',
    defaultOrder: 'created_at',
    searchableColumns: [],
    mapInput: (input) => {
...
// ---------------------------------------------------------------------------------------------
export const AGENDA_NOTIFICATION_PREFERENCES_RESOURCE = {
    schema: 'public',
    table: 'agenda_notification_preferences',
    returnRepresentation: true,
    select: 'id,notify_new_booking,notify_cancellation,notify_reminder,reminder_hours_before,channel_email,channel_push,channel_whatsapp,active,tenant_id,created_by,created_at,updated_at',
    primaryKey: 'id',
    defaultOrder: 'created_at',
    searchableColumns: [],
    mapInput: (input) => {
        ...
        if (input.channel_push !== undefined)
            payload.channel_push = toBool(input.channel_push, true);
...
export const resourceAgendaConfig = {
