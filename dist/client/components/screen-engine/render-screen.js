 ...
 */
export function RenderScreen({ config, context, }) {
    return (_jsx("div", { className: cn(
        ...
        'mx-auto flex w-full flex-1 flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8', config.maxWidth === 'narrow' ? 'max-w-4xl' : 'max-w-6xl'), children: config.blocks.map((block, index) => {
            const entry = SCREEN_COMPONENT_REGISTRY[block.component];
            if (!entry) {
                throw new Error(`screen-engine: componente "${block.component}" nao esta registrado em SCREEN_COMPONENT_REGISTRY (tela "${config.id}").`);
            }
            const Component = entry.component;
            const props = context ? resolveContextRefs(block.props, context) : block.props;
            return _jsx(Component, { ...props }, `${block.component}-${index}`);
        }) }));
}
