...
export function ChatShell({ renderContextBanner, } = {}) {
    const { user } = useAuth();
    ...
    _jsxs("div", { className: "flex min-h-0 flex-1 gap-4", children: [_jsx("div", { className: 'min-h-0 w-full lg:w-[340px] lg:shrink-0 ' + (active ? 'hidden lg:block' : 'block'), children: _jsx(ConversationList, { items: items, activeUid: activeUid, loading: loading, onSelect: select, onRefresh: refresh }) }), _jsx("div", { className: 'min-h-0 min-w-0 flex-1 ' + (active ? 'block' : 'hidden lg:block'), children: active ? (_jsx(ChatWindow, { conversation: active, me: me, onBack: () => setActiveUid(null), renderContextBanner: renderContextBanner })) : (_jsx("div", { className: "hidden h-full items-center justify-center rounded-2xl border border-border bg-card text-sm text-muted-foreground lg:flex", children: "Selecione uma conversa" })) })] }));
}
