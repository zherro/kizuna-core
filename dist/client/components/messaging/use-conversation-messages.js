...
export function useConversationMessages(uid) {
    ...
    const merge = useCallback((incoming) => {
        setMessages((prev) => {
            ...
            if (!add.length)
                return prev;
            const next = [...prev, ...add].sort((a, b) => a.id - b.id || a.createdAt.localeCompare(b.createdAt));
    ...
    useEffect(() => {
        ...
        const schedule = () => {
            ...
            if (backoffRef.current > 0)
                delay = cfg.pollBackoffMs[backoffRef.current - 1];
            else
                delay = Date.now() - lastActivityRef.current < 60000 ? cfg.pollActiveMs : cfg.pollIdleMs;
    ...
    const doSend = useCallback(async (text, token) => {
        if (!uid)
            return;
        setSending(true);
        try {
            ...
            lastActivityRef.current = Date.now();
            if (saved.id > lastIdRef.current)
                lastIdRef.current = saved.id;
        }
        catch {
            setMessages((prev) => prev.map((m) => (m.clientToken === token ? { ...m, status: 'failed' } : m)));
        }
        finally {
            setSending(false);
        }
    ...
    const retry = useCallback((token) => {
        if (!token)
            return;
        const target = messages.find((m) => m.clientToken === token);
        if (!target?.content)
            return;
        setMessages((prev) => prev.map((m) => (m.clientToken === token ? { ...m, status: 'pending' } : m)));
...
//# sourceMappingURL=use-conversation-messages.js.map