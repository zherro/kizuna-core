...
describe('useWizardConversation', () => {
    it('adapter ausente / unavailable → inativo', () => {
        const a = renderHook(() => useWizardConversation({ ...baseOpts, adapter: undefined, ctx: makeCtx() }));
        expect(a.result.current.active).toBe(false);
        const b = renderHook(() => useWizardConversation({
            ...baseOpts,
    ...
    it('advance:ask NÃO avança sozinha — só oferece a tag "Pode seguir"', async () => {
        ...
        const { result } = renderHook(() => useWizardConversation({ ...baseOpts, adapter, ctx: makeCtx(), goContinue }));
        await act(async () => {
    ...
    it('pickChoice "Pode seguir" chama goContinue; tag comum envia o valor', async () => {
        ...
        await waitFor(() => expect(converse).toHaveBeenCalledWith(expect.objectContaining({ userText: 'no_cliente' })));
    });
    it('passo avança pra frente (rodapé ou tag) → confirm-advance abre a pergunta do passo novo', async () => {
        ...
        const adapter = makeAdapter({ converse });
        const { result, rerender } = renderHook(({ ci }) => useWizardConversation({ ...baseOpts, adapter, ctx: makeCtx(), currentIndex: ci }), { initialProps: { ci: 0 } });
        ...
        await waitFor(() => expect(converse).toHaveBeenLastCalledWith(expect.objectContaining({ intent: 'confirm-advance', stepKey: 'more' })));
    });
    it('avançar sem ter conversado NÃO invoca a Naví', async () => {
        ...
        const adapter = makeAdapter({ converse });
        const { rerender } = renderHook(({ ci }) => useWizardConversation({ ...baseOpts, adapter, ctx: makeCtx(), currentIndex: ci }), { initialProps: { ci: 0 } });
        await act(async () => {
            rerender({ ci: 1 });
        });
    ...
    it('voltar um passo (currentIndex menor) NÃO dispara confirm-advance', async () => {
        const converse = vi
            ...
            .mockResolvedValue({ message: 'x', choices: [], patch: {}, advance: 'hold' });
        const adapter = makeAdapter({ converse });
        const { result, rerender } = renderHook(({ ci }) => useWizardConversation({ ...baseOpts, adapter, ctx: makeCtx(), currentIndex: ci }), { initialProps: { ci: 1 } });
        await act(async () => {
            await result.current.send('oi');
        });
        converse.mockClear();
...
//# sourceMappingURL=use-wizard-conversation.test.js.map