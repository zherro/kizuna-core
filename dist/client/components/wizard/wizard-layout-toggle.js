...
export function WizardLayoutToggle({ value, onChange, className, }) {
    return (_jsx("div", { role: "radiogroup", "aria-label": "Formato do formul\u00E1rio", className: cn('wz-layout-toggle inline-flex items-center gap-0.5 rounded-full p-0.5', className), children: OPTIONS.map(({ value: v, label, hint, Icon }) => {
            ...
            return (_jsxs("button", { type: "button", role: "radio", "aria-checked": active, title: hint, "data-active": active, onClick: () => !active && onChange(v), className: cn('inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-medium transition-colors sm:px-2.5 sm:py-1', active ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'), children: [_jsx(Icon, { className: "h-[18px] w-[18px] sm:h-3.5 sm:w-3.5", "aria-hidden": true }), _jsx("span", { className: "hidden sm:inline", children: label })] }, v));
        }) }));
}
