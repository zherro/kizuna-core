...
export const SERVICE_WIZARD_STEPS = {
    ...
    category: {
        ...
        persist: async (ctx) => {
            ...
            const prevLinks = ctx.entities.serviceSubcategoryLinks ?? [];
            const next = await syncServiceSubcategories(serviceId, groupId, categoryId, subcategoryIds, prevLinks);
            ctx.entities.serviceSubcategoryLinks = next;
        },
    },
    location: {
        key: 'location',
    ...
    'dynamic-form': {
        ...
        enabled: (ctx) => {
            const cat = dynamicFormCategory(ctx);
        ...
        persist: async (ctx) => {
            ...
            if (!ok)
                throw new Error('Nao foi possivel salvar as respostas do formulario.');
        },
    },
    moderation: {
        key: 'moderation',
        ...
        persist: async (ctx) => {
            ...
            if (serviceId == null)
                throw new Error('Servico sem id para moderar.');
...
export { moderateService } from './moderate-service';
