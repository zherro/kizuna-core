 ...
 */
export async function syncServiceSubcategories(serviceId, groupId, categoryId, subIds, prevLinks) {
    const noop = () => { };
    ...
    if (toRemove.length > 0) {
        const results = await Promise.all(toRemove.map(async (link) => {
    ...
    if (toAdd.length > 0) {
        const results = await Promise.all(toAdd.map((categorySubId) => submitResource({
...
//# sourceMappingURL=sync-service-subcategories.js.map