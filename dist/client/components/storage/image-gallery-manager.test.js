...
describe('ImageGalleryManager', () => {
    it('renderiza com um controle de upload', () => {
        render(_jsx(ImageGalleryManager, { referenceId: "r1", initialImageIds: [], onSaved: vi.fn(), onPersist: vi.fn(async () => []) }));
    ...
    it('ao enviar arquivo chama onPersist(referenceId, [id]) e onSaved com a lista normalizada', async () => {
        const onSaved = vi.fn();
        const onPersist = vi.fn(async (_ref, ids) => ids);
        const { container } = render(_jsx(ImageGalleryManager, { referenceId: "r1", initialImageIds: [], onSaved: onSaved, onPersist: onPersist }));
...
//# sourceMappingURL=image-gallery-manager.test.js.map