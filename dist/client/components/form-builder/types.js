...
export const DEFAULT_GRID = { xs: 12, sm: 12, md: 6, lg: 6, xl: 6, '2xl': 6 };
...
export function createField(type) {
    ...
    if (type === 'heading')
        base.label = 'Título da seção';
    if (type === 'info')
        base.label = 'Texto informativo';
    if (type === 'divider')
        base.label = '';
    return base;
}
 ...
 * an invalid save.
 */
export function collectKeyIssues(schema) {
    const issues = {};
    const seen = new Map();
