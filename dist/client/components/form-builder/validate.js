...
export function evalVisibleWhen(rule, values) {
    ...
    switch (rule.op) {
        ...
        case 'in': {
            ...
            if (Array.isArray(current))
                return current.some((c) => set.includes(c));
 ...
 */
export function collectOutput(schema, values) {
    const out = {};
...
export function validate(schema, values) {
    ...
    for (const field of schema.fields) {
        ...
        if (field.behavior.required) {
            const emptyForRequired = field.type === 'checkbox' || field.type === 'switch' ? !value : isEmpty(value);
            if (emptyForRequired) {
