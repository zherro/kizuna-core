...
function RatingChip({ rating, reviewCount = 0, className, }) {
    if (rating == null || reviewCount <= 0)
        return null;
    return (_jsxs("span", { className: cn('inline-flex items-center gap-1 text-[11px] font-semibold', className), children: [_jsx(Star, { className: "h-3 w-3 fill-current text-amber-500" }), rating.toFixed(1).replace('.', ','), _jsxs("span", { className: "font-normal text-muted-foreground", children: ["(", reviewCount, ")"] })] }));
}
