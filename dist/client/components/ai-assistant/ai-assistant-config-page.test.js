...
describe('AiAssistantConfigPage', () => {
    it('renderiza select de provedor e um toggle por contexto', () => {
        render(_jsx(AiAssistantConfigPage, { contexts: ['search', 'service-wizard'] }));
    ...
    it('opções openai/claude aparecem desabilitadas', () => {
        render(_jsx(AiAssistantConfigPage, { contexts: [] }));
        const openai = screen.getByRole('option', { name: /openai/i });
        const claude = screen.getByRole('option', { name: /claude/i });
        expect(openai.disabled).toBe(true);
        expect(claude.disabled).toBe(true);
    });
    it('mostra aviso quando statusConfigured é false', () => {
        render(_jsx(AiAssistantConfigPage, { contexts: [], statusConfigured: false }));
        expect(screen.getByRole('alert')).toBeDefined();
    });
});
//# sourceMappingURL=ai-assistant-config-page.test.js.map